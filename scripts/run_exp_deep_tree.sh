#!/usr/bin/env bash
# Deep-tree compute Pareto: matches SPO and InGPO at D in {2,3,4,5}.
# Demonstrates the break-even formula:
#     T_InGPO / T_SPO  ~  rho^(D-1) * kappa
# i.e. InGPO wins more as D grows and the savings rho^(D-1) shrink kappa
...
for d in ${DEPTHS}; do
  ...
  APP_EXPERIMENT_NAME="${TAG}-spo-d${d}" \
    ...
    --configs "${INGPO_ROOT}/configs/episode_generators/${tree_cfg}.jsonnet"

  # InGPO at the same depth.
  CFGS="${INGPO_ROOT}/configs/polIter_qwen1_5b_base_ingpo_tree_MATH.jsonnet"
  CFGS+=",${INGPO_ROOT}/configs/episode_generators/${tree_cfg}.jsonnet"
  APP_EXPERIMENT_NAME="${TAG}-ingpo-d${d}" \
    ingpo_run "${TAG}-ingpo-d${d}" "${CFGS}" "$@"
done
