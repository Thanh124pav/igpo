...
#   For each (Model x Dataset x Tree size), train SPO baseline + InGPO and
...
run_one() {
  ...
  if [[ "${model}" == "qwen" ]]; then
    ...
    INGPO_TREE="${tree}" APP_EXPERIMENT_NAME="${TAG}-spo-tree-${tree}-rho1.1b-gsm8k" \
      bash "${INGPO_ROOT}/scripts/run_baseline.sh" spo_tree_GSM8K
    INGPO_TREE="${tree}" APP_EXPERIMENT_NAME="${TAG}-ingpo-tree-${tree}-rho1.1b-gsm8k" \
      bash "${INGPO_ROOT}/scripts/train_ingpo_tree_GSM8K.sh"
  fi
}
