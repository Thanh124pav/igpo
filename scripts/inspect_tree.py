"""Pretty-print one InGPO tree JSON with per-node action / TV / gap stats.
...
def walk(node: Dict[str, Any], depth: int = 0) -> None:
    ...
    seg_id = node.get("ingpo_segment_id", "<root>")
    reward = node.get("reward")
    avg_lp_K = node.get("ingpo_avg_lp_K")
    tv = node.get("ingpo_tv_m")
    gap = node.get("ingpo_gap_m")
    target = node.get("ingpo_share_target")
...
def main() -> int:
    ...
    stats = tree.get("ingpo_stats", {}) if isinstance(tree, dict) else {}
    if stats:
        print("InGPO stats:")
