...
Reads a saved trajectory pickle from an InGPO run (with
...
def main():
    ...
    ap.add_argument("--ingpo_trajectories", required=True)
    ap.add_argument("--spo_trajectories", required=True)
    ap.add_argument("--epsilon", type=float, default=0.02)
    args = ap.parse_args()

    ingpo = load_pickle(Path(args.ingpo_trajectories))
    ...
    for tr in ingpo:
        ...
        for sid, node in index_by_seg(tr.get("tree", tr)).items():
            ...
            if action == "share":
                total_share += 1
                tgt = node.get("ingpo_share_target")
