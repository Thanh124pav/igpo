...
#   InGPO tree 3-9-27, SPO tree 3-9-27, GRPO with 27 rollouts.
#
...
GRPO_CFG="${INGPO_ROOT}/configs/local/grpo_27rolls.jsonnet"
...
run_one() {
  ...
  APP_EXPERIMENT_NAME="${TAG}-${name}" ingpo_run "${TAG}-${name}" "${cfgs}" "$@"
}

run_one "ingpo-tree-3_9_27" "$(resolve_math_config ingpo_tree "${MODEL}"),${TREE_CFG},${SUBSET_CFG},${RUNTIME_CFG}" "$@"
