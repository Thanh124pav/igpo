...
## Compare InGPO, SPO, GRPO

Run InGPO tree 3-9-27, SPO tree 3-9-27, and GRPO with 27 rollouts:

```bash
conda activate deeplearning
WANDB_PROJECT=igpo-local \
MODEL=qwen3_0_6b \
MATH_LOCAL_SIZE=10 \
...
- `INGPO_GPU=1` or `INGPO_GPUS=0,1`.
- `INGPO_TOTAL_NUM_ITERATIONS=1` for the shortest run.
...
Run the SPO-tree-shaped InGPO budget-allocation path, where TV estimates are
...
```text
experiments/<exp_name>/ingpo_demos/reward_variance_nodes.jsonl
experiments/<exp_name>/ingpo_demos/reward_variance_nodes.csv
```

Each row contains `reward`, `reward_std`,
`empirical_child_reward_variance`, `ingpo_reward_variance`,
`ingpo_sigma2`, `ingpo_sigma4`, TV support counts, and allocated branch budget
for one node. The wandb run also gets summary metrics under
`ingpo/reward_variance_nodes/*`.
