...
InGPO: Cắt tỉa và share value **ngay khi segment sinh ra**, trước khi expand con. Giảm variance gradient PPO.
...
InGPO kế thừa SPO: build tree theo depth, width W song song. Khác biệt: mỗi node sinh ra sẽ trigger check Share/Prune **ngay lập tức** trước khi cho phép expand con.

```
Algorithm: InGPO Training for one problem
...
InGPO giảm số segment cần update từ `O(W^D)` của SPO xuống `O((ρW)^D)`, với `ρ<1` là tỉ lệ giữ lại sau prune/share.
...
V1: ValueShare chỉ với `pa(s)`. V2: với `root`. V3: với `nearest` InGPO.
...
V1: Y cố định cho cả dataset. V2: Y per-problem như InGPO.
...
**Abl 7: Oracle**
Train SPO không prune, sau đó check các segment mà InGPO đã prune/share: nếu update policy trên chúng thì Δacc?
...
### 7. Contributions
1. **Algorithm**: InGPO, online segment-level pruning/sharing cho SPO, trigger ngay khi segment sinh.
